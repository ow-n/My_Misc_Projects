...
void SchedulerSJF::init(std::vector<PCB>& process_list) {
    ...
    // Selection Sort Original Process_List on Burst Time and ID
    for (size_t i = 0; i < this->process_list.size() - 1; i++) { // iterate through process_list
        ...
        for (size_t j = i + 1; j < this->process_list.size(); j++) { // iterate through unsorted array
            if (this->process_list[j].burst_time < this->process_list[min_idx].burst_time) { // sort by burst_time
                min_idx = j; // find min ele
            } else if (this->process_list[j].burst_time == this->process_list[min_idx].burst_time) { // if burst_time is same
