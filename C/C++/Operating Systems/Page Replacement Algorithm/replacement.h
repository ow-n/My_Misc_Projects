...
protected:  // subclasses can access these members
    ...
    int num_frames; // Total number of physical memory frames
    int num_pages;  // Total number of logical pages
    std::vector<bool> frame_available; // Vector to Track available frames
...
public:
    ...
    /**
     * @brief Get the number of page faults
     */
    int getFreeFrame();

    /**
     * @brief Set the number of free frames
     */
    void setFreeFrame(int frame_num);
};
